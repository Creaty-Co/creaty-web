...
export function CheckTree<V>(props: ICheckTree<V>) {
  ...
  return (
    <div className="check-tree">
      <div className="check-tree-show">
        {values.map((value, index) => (
          <div className="check-tree-show__check" key={index}>
            ...
            <Tag noHash onClick={() => uncheck(value)}>
              {String(
                (children as any)
                  .flatMap((d: any) =>
                    d?.props?.children && d?.type === "option" ? [d, ...toArrayDeeply(d.props.children)] : d
                  )
                  .find((option: any) => option?.props?.value === value)?.props?.title
              )}
            </Tag>
            {/* --- Trash Code --- */}
      ...
      <div className="check-tree__list">
        <CheckTreeColumn>
          {children.map(
            selectOptionPredicate((child, index) => (
              <CheckTreeRow
                active={cursor[0] === index}
                expanded={!!child.props.children}
                onClick={() => (child.props.children ? setCursor([index]) : undefined, onCheck(child.props.value as V))}
                key={index}
              >
                {child.props.value == null && child.props.title}
                {!!child.props.value && (
                  <Checkbox
                    value={child.props.value}
                    checked={values.includes(child.props.value as V)}
        ...
        {cursor.map(columnIndex => (
          <CheckTreeColumn key={columnIndex}>
            {toArrayDeeply((children[columnIndex] as CheckTreeOption<V>).props.children).map(
              selectOptionPredicate((child, index) => (
                <CheckTreeRow
                  active={cursor[columnIndex + 1] === index}
                  expanded={!!child.props.children}
                  onClick={() => (
                    child.props.children ? setCursor([...cursor.slice(0, columnIndex + 1), index]) : undefined,
                    onCheck(child.props.value as V)
                  )}
                  key={index}
                >
                  <Checkbox
...
function selectOptionPredicate<V, T extends CheckTreeOption<V> | ReactNode, U>(
  predicate: (value: CheckTreeOption<V>, index: number, array: T[]) => U
) {
...
interface ICheckTreeColumn {
  children: ReactNode | ReactNode[]
}

function CheckTreeColumn(props: ICheckTreeColumn) {
  return (
...
function CheckTreeRow(props: ICheckTreeRow) {
  return (
    <div
      className={classWithModifiers("check-tree-row", props.active && "active")}
      role={props.expanded ? "option" : undefined}
      onClick={props.onClick}
    >
      <div className="check-tree-row__title">{props.children}</div>
      {props.expanded && <Icon className="check-tree-row__icon" name="chevron" />}
    </div>
  )
}
